...
class ServeClientFasterWhisper(ServeClientBase):
    ...
    def __init__(
        ...
        compute_override = os.environ.get("WHISPER_COMPUTE_TYPE")
        if compute_override:
            self.compute_type = compute_override
        elif device == "cuda":
    ...
    def transcribe_audio(self, input_sample):
        ...
        """
        # Energy pre-check: skip obviously silent chunks before they reach
        # the batch queue or lock. ~0.01ms, no neural network needed.
        # Full Silero VAD still runs inside transcribe()/batch for proper filtering.
        if self.use_vad:
            rms = float(np.sqrt(np.mean(input_sample ** 2)))
            if rms < 0.003:
                return None
