#!/usr/bin/env python3
"""E2E pipeline test — Fixed window flushing (no VAD).

Aggressively flushes every ~5 seconds, waiting for the current word to complete.
Uses higher LLM context window (5 pairs) to compensate for mid-sentence cuts.

Usage:
  python scripts/test_e2e_window.py --desktop
  python scripts/test_e2e_window.py --desktop --monitor-source firefox_capture.monitor
  python scripts/test_e2e_window.py --desktop --no-tts
"""
...
async def run_e2e(args):
    ...
    print("=== E2E Pipeline Test: Window flush + VAD buffer clear ===")
    ...
    print(f"Window: {args.window}s, Context: {CONTEXT_WINDOW} pairs")
    print("─" * 50)
    ...
    async with websockets.connect(STT_WS_URL) as ws:
        ...
        async def receive_transcripts():
            nonlocal completed_count, last_partial, send_seq, flushed_len
            ...
            async for message in ws:
                ...
                for i, seg in enumerate(segments):
                    ...
                    if completed and i >= completed_count:
                        ...
                        # Window flush: flush every ~N seconds
                        # Flush up to the last complete word (exclude the
                        # final token which may still be changing)
                        if elapsed >= args.window and unflushed:
                            words = text.rsplit(maxsplit=1)
                            flush_text = words[0] if len(words) > 1 else text
                            do_flush(flush_text, f"window {elapsed:.1f}s")
                            last_partial = ""
                            flushed_len = 0
                            window_start = time.monotonic()
                            await ws.send(json.dumps({
                                "type": "clear_buffer",
                                "seq": send_seq,
                                "context": flush_text[-100:],  # last 100 chars as decoder context
                            }))
                        else:
                            # Display only unflushed portion
                            display_text = unflushed if unflushed else text
                            max_display = terminal_width - 12
                            display = display_text if len(display_text) <= max_display else display_text[:max_display - 3] + "..."
                            clear_line()
...
def main():
    parser = argparse.ArgumentParser(description="E2E pipeline test: Fixed window flushing (no VAD)")
    ...
    parser.add_argument("--tts-sink", type=str, default=None, help="PipeWire/PulseAudio sink name for TTS output")
    parser.add_argument("--list-devices", action="store_true", help="List audio devices")
    parser.add_argument("--window", type=float, default=5.0, help="Flush window in seconds (default: 5.0)")
