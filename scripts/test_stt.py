#!/usr/bin/env python3
"""Interactive STT test — streams mic or desktop audio to WhisperLive and
displays partial/completed transcriptions in real-time.

Tests:
  1. Raw STT output (partials + completions from WhisperLive)
  2. Stable-prefix flushing logic (orchestrator-level)

Usage:
  python scripts/test_stt.py                    # mic, raw STT output
  python scripts/test_stt.py --with-flushing    # mic, with stable-prefix flushing
  python scripts/test_stt.py --desktop          # capture desktop/YouTube audio
  python scripts/test_stt.py --list-devices
  python scripts/test_stt.py --input-device 4
"""
...
async def run_with_flushing(args):
    ...
    def do_flush(text: str, reason: str):
        ...
        flush_num += 1
        clear_line()
        print(f"  ★ FLUSH #{flush_num} ({reason}): {text}")
